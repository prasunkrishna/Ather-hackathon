...
// ======= Braking detection (your choice: A) =======
...
float brakeBelowTimerMs = 0.0f;       // accumulates time below harsh threshold
int   brake_event = 0;                // 0 none, 1 normal, 2 harsh
unsigned long lastBrakeAnnounceMs = 0;

// ======= Motion state (IMU-only) =======
enum MotionState { IDLE=0, WALK=1, RIDE=2 };
MotionState currState = IDLE;

// Sliding window over |linear accel| magnitude for stddev
// ~2.8 s window at 350 ms/log cadence, but we update every loop (~100 Hz)
// We'll accumulate into a fixed-length ring buffer at ~25 Hz for efficiency.
const int SW_N = 40;                   // ~1.6 s at 25 Hz; we double count to ~3.2 s by smoothing
...
void handleDownload() {
  String csv = "ms,pitch,roll,ax_f,ay_f,az_f,lx,ly,lz,brake_event,state\r\n";
  size_t n = sampleCount;
  for (size_t i=0;i<n;i++){
...
void loop() {
  ...
  // Gravity components from full tilt (roll φ, pitch θ)
  ...
  if (now - lastLogMs >= 350) {
    ...
    if (sampleCount < MAX_SAMPLES) {
      samples[sampleCount++] = { now, pitch, roll, ax_f, ay_f, az_f, lx, ly, lz, brake_event, (int)currState };
    }
  }
